...
const DEFAULT_SUBTITLE = 'Bambu İpek, Saf Yün, Otel ve Camii koleksiyonları stok takibi ve ürün revizyonu';
const DEFAULT_CATEGORIES = ['Bambu İpek', 'Proje/Otel', 'Klasik', 'Camii', 'Modern', 'Özel Koleksiyon'];
...
export const ProductCatalogView: React.FC<ProductCatalogViewProps> = ({
  ...
  const [formCategory, setFormCategory] = useState<string>('Bambu İpek');

  // Manual Barcode & Catalog Fields (1-Koleksiyon, 2-Desen, 3-Ölçü, 4-Alış Fiyatı & KDV)
  const [formCollectionName, setFormCollectionName] = useState<string>('Bambu İpek Koleksiyonu');
  ...
  const handleOpenAddModal = () => {
    ...
    setFormCategory(customCategories[0] || 'Bambu İpek');
    setFormCollectionName(customCategories[0] || 'Bambu İpek Koleksiyonu');
  ...
  const handleOpenEditModal = (p: CarpetProduct) => {
    ...
    setFormCategory(p.category || customCategories[0] || 'Bambu İpek');
    setFormCollectionName(p.collectionName || p.category || 'Bambu İpek Koleksiyonu');
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white border border-slate-200 p-5 rounded-xl shadow-sm relative group">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <button
            ...
            className="bg-slate-100 hover:bg-rose-50 hover:text-rose-700 text-slate-700 border border-slate-300 font-bold text-xs px-3.5 py-2.5 rounded-xl transition-all flex items-center gap-1.5 cursor-pointer"
            title="Sistemle gelen varsayılan demo ürünleri siler"
          >
            <Trash2 className="w-3.5 h-3.5 text-rose-500" />
            <span>Demo Ürünleri Sil</span>
          </button>
      ...
      {filteredProducts.length === 0 && (
        <div className="p-12 text-center bg-white border border-dashed border-slate-300 rounded-2xl space-y-3">
          ...
          <p className="text-xs text-slate-500 max-w-md mx-auto">
            Seçtiğiniz filtreye uygun ürün yok veya tüm demo ürünleri sildiniz. Yeni ürün eklemek için aşağıdaki butonu kullanabilirsiniz.
          </p>
          <button
            onClick={handleOpenAddModal}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredProducts.map((prod) => {
          ...
          return (
            <div
              key={prod.id}
              className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-xs hover:border-indigo-300 hover:shadow-md transition-all flex flex-col justify-between group"
            >
              {/* Image Banner */}
              <div className="relative h-48 w-full overflow-hidden bg-slate-100">
                ...
                <div className="absolute top-3 left-3 flex items-center gap-1.5 flex-wrap">
                  <span className="bg-slate-900/90 text-white backdrop-blur-md font-mono font-bold text-[10px] px-2.5 py-1 rounded-lg shadow-2xs">
                    {prod.code}
                  </span>
                  {isDemo && (
                    <span className="bg-amber-100 text-amber-800 border border-amber-300 font-bold text-[9px] px-2 py-0.5 rounded">
                      Demo
                    </span>
                  )}
                </div>

                <span className="absolute bottom-3 right-3 bg-indigo-600 text-white font-mono font-extrabold text-xs px-2.5 py-1 rounded-lg shadow-md">
              ...
              <div className="p-4 space-y-3 flex-1 flex flex-col justify-between">
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    {prod.barcode && (
                      <span className="text-[10px] font-mono text-slate-500 flex items-center gap-1 bg-slate-50 px-1.5 py-0.5 rounded border border-slate-200">
                  ...
                  <p className="text-xs text-slate-500 mt-1 line-clamp-2">{prod.description || 'Açıklama girilmemiş.'}</p>
                </div>

                {/* Specs */}
                <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 p-3 rounded-xl border border-slate-200 font-mono">
                  <div>
                    <span className="text-slate-400 block font-sans">Hav Yüksekliği:</span>
                    <span className="text-slate-900 font-bold">{prod.pileHeightMm || 10} mm</span>
                  </div>
                  <div>
                    <span className="text-slate-400 block font-sans">İlme Sıklığı:</span>
                    <span className="text-slate-900 font-bold">
                      {prod.densityPoints ? prod.densityPoints.toLocaleString('tr-TR') : '-'}
                    </span>
                  </div>
                  <div>
                    <span className="text-slate-400 block font-sans">Mevcut Stok:</span>
                    <span className="text-emerald-600 font-bold">{prod.stockM2 || 0} m²</span>
                  </div>
                  <div>
                    <span className="text-slate-400 block font-sans">Renk Seçeneği:</span>
                    <span className="text-slate-900 font-bold">{prod.colorVariants ? prod.colorVariants.length : 1} Renk</span>
                  </div>
                </div>
                ...
                <div className="pt-2 space-y-2 border-t border-slate-100">
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className="w-full bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold py-2 rounded-lg border border-slate-300 flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                    ...
                    <button
                      ...
                      className="w-full bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold py-2 rounded-lg border border-rose-200 flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                  ...
                  <button
                    ...
                    className="w-full bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold py-2 rounded-lg border border-indigo-200 flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                  >
                    <Sparkles className="w-3.5 h-3.5 text-indigo-600" />
                    <span>Bu Ürüne Fiyat Hesapla</span>
                  </button>
                </div>
              </div>
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    placeholder="ör: PulCarpet SilkTouch Bambu İpek"
